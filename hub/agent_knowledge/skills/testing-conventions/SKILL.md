...
- **`webapp-testing`** (vendored from Anthropic) — a concrete Playwright toolkit
  for driving and asserting against a running local web app (screenshots,
  console capture, network). This skill is the language-agnostic philosophy;
  that one is the hands-on browser-testing implementation.
